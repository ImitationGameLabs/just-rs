...
//! fields the generic OpenAI-compatible surface cannot express (multimodal content parts,
//! `top_k`), which surface as an explicit invalid-request error rather than being silently
//! dropped.
//!
    ...
    match content {
        ...
        client_gen::MessageContent::Parts(parts) => match parts.as_slice() {
            ...
            _ => Err(BackendError::invalid_request(
                "OpenAI-compatible providers do not support multimodal message content",
            )),
        },
    }
}
